...
- **Districts.** The map is zoned residential, commercial and industrial, and a building's kind is
  chosen from what suits its district. A warehouse cannot appear between two houses, which is what
  stops the town reading as a patchwork.
- **Building archetypes** — house, townhouse, shop, office, warehouse and depot — each with its own
  proportions, wall material, interior surface, window density and habits about being subdivided.
- **Interior rooms**, generated by recursive bisection with a doorway through every dividing wall,
  so no room can be sealed off.
- **Roofs**, which lift off whichever building you are standing in.
- A seeded random number generator, so a given seed always produces the same town and a layout bug
